...
- `src/components/playback/MusicPlayer.tsx` + `musicTracks.ts` - the crys (Ellis) soundtrack layered over the game audio: own `<audio>`, independent volume (default 10%), shuffle (on)/repeat/seek, track list, animated equaliser, and a floating now-playing chip. Opens on a random track and starts at GO via the `playheadPlaying` prop (the countdown's play-start). Toggled by the `music` icon in the theater control bar. Tracks served from `public/music/` (gitignored; deployed via scp), manifest committed in `musicTracks.ts`.
- Noclip saved camera views: `FreeCamera` (in `CinematicScene.tsx`) accepts `savedCams`/`captureCue`/`onCaptureCam`/`gotoCam`; `PlaybackHub` owns the `SavedCam[]` list + the top-centre toolbar. Save the current free-fly pose and recall it from a numbered chip or the number keys 1–9 (max 9 slots).
